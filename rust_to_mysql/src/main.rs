...
fn main() -> Result<()> {
    ...
    let result = conn.query_map(
        ...
        |(bookid, bookname, publisher, price)| Book {
            bookid,
            bookname,
            publisher,
            price,
        },
    )?; // 보낸 쿼리에 대한 결과 mapping
    for r in result {
        println!(
            "{} {} {} {}",
            r.bookid,
            r.bookname.unwrap_or("NULL".to_string()),
            r.publisher.unwrap_or("NULL".to_string()),
            r.price.unwrap_or_default()
        );
    ...
    // 테이블에 데이터 삭제
    println!("\nDelete row in table");
    conn.query_drop(r"delete from Book where bookid='99'")?;
